...
const getTokenWhiteList = createAsyncThunk(`${SLICE_NAME}/getTokenWhiteList`, async (account_id) =>
  fiatValueManager.fetchTokenWhiteList(account_id),
);
...
const tokenFiatValuesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addMatcher(
      isAnyOf(fetchCoinGeckoFiatValues.fulfilled, fetchRefFinanceFiatValues.fulfilled),
      (state, action) => {
