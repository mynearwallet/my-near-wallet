...
export const handleSignPrivateShardTransactions = createAsyncThunk(
  ...
  async ({ customRPCUrl, xApiToken }, thunkAPI) => {
    ...
    const transactions = retryingTx
      ? selectSignRetryTransactions(getState())
      : selectSignTransactions(getState());
    const accountId = selectAccountId(getState());

    try {
      transactionsHashes = await wallet.signAndSendCalimeroTransaction(
        transactions,
        accountId,
        customRPCUrl,
        xApiToken,
...
export const selectSignCallbackUrl = createSelector([selectSignSlice], (sign) => sign.callbackUrl);
...
export const selectSignErrorName = createSelector([selectSignError], (error) => error?.name);

export const selectSignErrorMessage = createSelector([selectSignError], (error) => error?.message);
