...
const fetchNumberOfOwnedNFTsForContract = createAsyncThunk(
  ...
  async ({ accountId, contractName, contractMetadata }, thunkAPI) => {
    ...
    const numberOfOwnedTokens = parseInt(
      await getNumberOfTokens({ accountId, contractName, contractMetadata }),
    );
...
const fetchOwnedNFTsForContract = createAsyncThunk(
  ...
  async ({ accountId, contractName, contractMetadata }, thunkAPI) => {
    ...
    const tokenMetadata = await getTokens({
      ...
      fromIndex: selectTokensListForAccountForContract(getState(), { accountId, contractName })
        .length,
    });
...
const fetchNFT = createAsyncThunk(
  ...
  async ({ accountId, contractName, tokenId }, { dispatch, getState }) => {
    ...
    if (
      !selectTokenForAccountForContractForTokenId(getState(), { accountId, contractName, tokenId })
    ) {
      const contractMetadata = await getCachedContractMetadataOrFetch(contractName, getState());
...
const fetchNFTs = createAsyncThunk(`${SLICE_NAME}/fetchNFTs`, async ({ accountId }, thunkAPI) => {
  ...
  await Promise.all(
    likelyContracts.map(async (contractName) => {
      ...
      try {
        ...
        await dispatch(fetchOwnedNFTsForContract({ accountId, contractName, contractMetadata }));
        await dispatch(
          fetchNumberOfOwnedNFTsForContract({ accountId, contractName, contractMetadata }),
        );
      } catch (e) {
...
const nftSlice = createSlice({
  ...
  reducers: {
    ...
    addNumberOfOwnedTokens(state, { payload }) {
      ...
      set(
        state,
        ["ownedTokens", "byAccountId", accountId, "numberByContractName", contractName],
        numberOfOwnedTokens,
      );
    },
    transferToken(state, { payload }) {
      ...
      merge(state, {
        transferredTokens: { byContractName: { [contractName]: { [nft.token_id]: nft } } },
      });
      update(
        state,
        ["ownedTokens", "byAccountId", accountId, "byContractName", contractName, "tokens"],
  ...
  extraReducers: (builder) => {
    handleAsyncThunkStatus({
      ...
      buildStatusPath: ({ meta: { arg: { accountId, contractName } } }) => [
        "ownedTokens",
        "byAccountId",
        accountId,
        "byContractName",
...
const selectTransferredTokensSlice = createSelector(
  selectNftSlice,
...
export const selectTokenForAccountForContractForTokenId = createSelector(
  ...
  (tokensListByAccountByContract, tokenId) =>
    tokensListByAccountByContract.find(({ token_id }) => token_id === tokenId),
);
