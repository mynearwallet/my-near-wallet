...
export const createIdentityFundedAccount = createAsyncThunk(
    ...
    async (
        ...
        await dispatch(
            addLocalKeyAndFinishSetup({ accountId, recoveryMethod, publicKey })
        ).unwrap();
    }
);
...
export const createNewAccount = createAsyncThunk(
    ...
    async (
        ...
        if (fundingContract && fundingKey) {
            await wallet.createNewAccountLinkdrop(
                accountId,
                fundingContract,
                fundingKey,
                publicKey
            ...
            await wallet.createNewAccountFromAnother(
                accountId,
                fundingAccountId,
                publicKey
            );
...
export const createAccountFromImplicit = createAsyncThunk(
    ...
    async ({ accountId, implicitAccountId, recoveryMethod }, { dispatch }) => {
        ...
        await dispatch(
            createNewAccount({ accountId, fundingOptions, recoveryMethod, publicKey })
        ).unwrap();
    }
...
export const createNewAccountWithCurrentActiveAccount = createAsyncThunk(
    ...
    async (
        { newAccountId, implicitAccountId, newInitialBalance, recoveryMethod },
        { dispatch }
    ) => {
...
export const finishSetupImplicitAccount = createAsyncThunk(
    ...
    async ({ implicitAccountId, recoveryMethod }, { dispatch }) => {
        const publicKey = new PublicKey({
            keyType: KeyType.ED25519,
            data: Buffer.from(implicitAccountId, 'hex'),
        });
    ...
    async ({ implicitAccountId, recoveryMethod }, { dispatch }) => {
        try {
            if (recoveryMethod === 'ledger') {
                ...
                const fullAccessKeys = accessKeys.filter(
                    (it) => it.access_key?.permission === 'FullAccess'
                );
                if (fullAccessKeys.length === 1) {
                    const newKeyPair = KeyPair.fromRandom('ed25519');
                    const newPublicKey = newKeyPair.publicKey;
                    await wallet.addNewAccessKeyToAccount(
                        implicitAccountId,
                        newPublicKey
                    );
...
export const initiateSetupForZeroBalanceAccountLedger = createAsyncThunk(
    ...
    async ({ implicitAccountId, ledgerPublicKey, ledgerHdPath }, { dispatch }) => {
        try {
            try {
                await sendJson(
                    'POST',
