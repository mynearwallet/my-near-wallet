...
export const createNewAccountWithCurrentActiveAccount = createAsyncThunk(
  ...
  async ({ newAccountId, implicitAccountId, newInitialBalance, recoveryMethod }, { dispatch }) => {
...
export const finishSetupImplicitAccount = createAsyncThunk(
  ...
  async ({ implicitAccountId, recoveryMethod }, { dispatch }) => {
    ...
    await dispatch(
      addLocalKeyAndFinishSetup({ accountId: implicitAccountId, recoveryMethod, publicKey }),
    ).unwrap();
  },
);
