...
const handleShowConnectModal = createAsyncThunk(
  ...
  async (_, { dispatch }) => {
    dispatch(
      ledgerSlice.actions.setLedgerConnectionModalType({ type: CONNECT_MODAL_TYPE.CONNECT }),
    );
  },
);
...
const handleDisconnectLedger = createAsyncThunk(
  ...
  async (_, { dispatch }) => {
    ...
    dispatch(
      ledgerSlice.actions.setLedgerConnectionModalType({ type: CONNECT_MODAL_TYPE.DISCONNECTED }),
    );
...
const signInWithLedgerAddAndSaveAccounts = createAsyncThunk(
  ...
  async ({ path, accountIds }, { dispatch, getState }) => {
    ...
    return dispatch(
      saveAndSelectLedgerAccounts({ accounts: selectLedgerSignInWithLedger(getState()) }),
    );
  },
);
...
const ledgerSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state, { payload, meta: { arg: { accountId } } }) => {
        set(state, ["signInWithLedgerStatus"], LEDGER_MODAL_STATUS.CONFIRM_ACCOUNTS);
    ...
    builder.addCase(
      ...
      (state, { error, meta: { arg: { accountId } } }) => {
        set(state, ["signInWithLedgerStatus"], LEDGER_MODAL_STATUS.CONFIRM_ACCOUNTS);
    ...
    builder.addMatcher(
      ({ type, ready, error }) =>
        ready || error || type.endsWith("/rejected") || type.endsWith("/fulfilled"),
      (state, { type }) => {
