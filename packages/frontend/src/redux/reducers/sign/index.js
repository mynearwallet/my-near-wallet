...
const deserializeTransactionsFromString = (transactionsString) =>
  transactionsString
    ...
    .map((buffer) =>
      utils.serialize.deserialize(transaction.SCHEMA, transaction.Transaction, buffer),
    );
...
const handleTransactionsRejected = (state, { error }) => {
  ...
  const gasUsed = calculateGasForSuccessTransactions({
    transactions: transactions,
    successHashes: state.successHashes,
  });
  // remove transactions that succedeed, so it will no be sign again
  const retryTransactions = removeSuccessTransactions({
    transactions,
    successHashes: state.successHashes,
  });
  ...
  const transaction = getFirstTransactionWithFunctionCallAction({
    transactions: retryTransactions,
  });

  const canRetryWithIncreasedGas =
    notEnoughGasAttached &&
    hasAtLeastOneFunctionCallAction &&
    transaction &&
    checkAbleToIncreaseGas({ transaction });
  return {
    ...state,
    retryTransactions,
    gasUsed: new BN(state.gasUsed || "0").add(new BN(gasUsed || "0")),
    status: canRetryWithIncreasedGas ? SIGN_STATUS.RETRY_TRANSACTION : SIGN_STATUS.ERROR,
    error,
  };
};
...
const sign = handleActions(
  {
    [parseTransactionsToSign]: (
      state,
      { payload: { transactions: transactionsString, callbackUrl, meta } },
    ) => {
      ...
      return {
        ...
        sensitiveActionsCounter: allActions.filter(
          (a) => ["deployContract", "stake", "deleteAccount"].indexOf(Object.keys(a)[0]) > -1,
        ).length,
      };
    },
    [updateSuccessHashes]: (state, { payload }) => ({
      ...state,
      successHashes: [...(state.successHashes || []), ...payload],
    }),
