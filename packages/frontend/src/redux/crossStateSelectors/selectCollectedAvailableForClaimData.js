...
import { selectTokensFiatValueUSD, selectTokenWhiteList } from "../slices/tokenFiatValues";
...
const collectFarmingData = (args) => {
  try {
    ...
    const collectedBalance = filteredFarms.reduce(
      (acc, farm) => ({
        ...acc,
        [farm.token_id]: new BN(acc[farm.token_id]).add(new BN(farm.balance)).toString(),
      }),
      {},
    );
