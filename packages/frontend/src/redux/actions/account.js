...
import { handleStakingUpdateAccount, handleStakingUpdateLockup, handleGetLockup } from "./staking";
...
export const handleRefreshUrl = (prevRouter) => (dispatch, getState) => {
  ...
  if (
    ...
    if (
      [WALLET_CREATE_NEW_ACCOUNT_URL, WALLET_LINKDROP_URL, WALLET_VERIFY_OWNER_URL].includes(
        currentPage,
      ) &&
      search !== ""
    ) {
      ...
      [WALLET_LOGIN_URL, WALLET_SIGN_URL, WALLET_BATCH_IMPORT_URL].includes(currentPage) &&
      (search !== "" || hash !== "")
    ) {
      saveState(parsedUrl);
      dispatch(refreshUrl(parsedUrl));
...
const checkContractId = () => async (dispatch, getState) => {
  ...
  if (contractId) {
    const redirectIncorrectContractId = () => {
      ...
      dispatch(
        redirectTo(`/${WALLET_LOGIN_URL}/?invalidContractId=true&failure_url=${failureUrl}`, {
...
export const allowLogin = () => async (dispatch, getState) => {
  ...
  if (successUrl) {
    if (publicKey) {
      await dispatch(
        withAlert(addAccessKey(wallet.accountId, contractId, publicKey, false, methodNames), {
          onlyError: true,
        }),
      );
    }
...
export const {
  ...
  REMOVE_ACCESS_KEY: [wallet.removeAccessKey.bind(wallet), () => showAlert({ onlyError: true })],
...
const handleFundCreateAccountRedirect =
  ...
  (dispatch, getState) => {
    ...
    if (ENABLE_IDENTITY_VERIFIED_ACCOUNT) {
      ...
      dispatch(
        redirectTo(`/fund-create-account/${accountId}/${implicitAccountId}/${recoveryMethod}`),
      );
    }
  };
...
export const { addAccessKey, addAccessKeySeedPhrase } = createActions({
...
export const { recoverAccountSecretKey } = createActions({
  RECOVER_ACCOUNT_SECRET_KEY: [wallet.recoverAccountSecretKey.bind(wallet), () => showAlert()],
});
