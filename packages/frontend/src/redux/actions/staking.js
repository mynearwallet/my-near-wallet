...
export const { staking } = createActions({
  STAKING: {
      ...
      LOCKUP: [
        async (lockupId, amount, contract, validatorId) => {
          ...
          if (validatorId !== selectedValidatorId) {
            if (selectedValidatorId !== null) {
              await signAndSendTransaction({
                receiverId: lockupId,
                actions: [functionCall("unselect_staking_pool", {}, CONFIG.STAKING_GAS_BASE, "0")],
              });
            }
            await signAndSendTransaction({
              receiverId: lockupId,
          ...
          return await signAndSendTransaction({
            receiverId: lockupId,
            actions: [
              functionCall("deposit_and_stake", { amount }, CONFIG.STAKING_GAS_BASE * 5, "0"),
            ],
          });
        },
      ...
      ACCOUNT: [
        async (validatorId, amount, accountId, contract) => {
          const result = await signAndSendTransaction({
            ...
            actions: [functionCall("deposit_and_stake", {}, CONFIG.STAKING_GAS_BASE * 5, amount)],
          });
      ...
      LOCKUP: [
        async (lockupId, amount) => {
          if (amount) {
            return await signAndSendTransaction({
              receiverId: lockupId,
              actions: [functionCall("unstake", { amount }, CONFIG.STAKING_GAS_BASE * 5, "0")],
            });
          }
          return await signAndSendTransaction({
            receiverId: lockupId,
            actions: [functionCall("unstake_all", {}, CONFIG.STAKING_GAS_BASE * 5, "0")],
          });
        },
        () => showAlert({ onlyError: true }),
      ],
      ...
      ACCOUNT: [
        async (validatorId, amount) => {
          ...
          if (amount) {
            result = await signAndSendTransaction({
              receiverId: validatorId,
              actions: [functionCall("withdraw", { amount }, CONFIG.STAKING_GAS_BASE * 5, "0")],
            });
          } else {
            result = await signAndSendTransaction({
              receiverId: validatorId,
              actions: [functionCall("withdraw_all", {}, CONFIG.STAKING_GAS_BASE * 7, "0")],
            });
          }
          if (result === false) {
            throw new WalletError(
    ...
    UPDATE_ACCOUNT: async (balance, validators, accountId, validatorDepositMap) => {
      ...
      await Promise.all(
        validators.map(async (validator, i) => {
          try {
            const total = new BN(
              await validator.contract.get_account_total_balance({ account_id: accountId }),
            );
            if (total.lte(ZERO)) {
            ...
            validator.unstaked = new BN(
              await validator.contract.get_account_unstaked_balance({ account_id: accountId }),
            );
            ...
            const networkId =
              wallet.connection.provider.connection.url.indexOf(MAINNET) > -1 ? MAINNET : TESTNET;
    ...
    UPDATE_LOCKUP: async (contract, account_id, exAccountId, accountId, validators) => {
      ...
      try {
        ...
        if (total.gt(ZERO)) {
          ...
          validator.unstaked = new BN(
            await validator.contract.get_account_unstaked_balance({ account_id }),
          );
    ...
    GET_LOCKUP: [
      async ({ accountId }) => {
        ...
        try {
          ...
          contract = await new Contract(await wallet.getAccount(accountId), lockupId, {
            ...lockupMethods,
          });
        } catch (e) {
          return;
    ...
    GET_VALIDATORS: async (accountIds, accountId) => {
      ...
      if (!accountIds) {
        const rpcValidators = [...current_validators, ...next_validators, ...current_proposals].map(
...
export const getValidatorFarmData = createAsyncThunk(
  ...
  async ({ validator, accountId }, { dispatch }) => {
    ...
    try {
      await Promise.all(
        farmList.map(({ token_id }) => dispatch(fetchToken({ contractName: token_id }))),
      );
    } catch (error) {
      console.error(error);
    }
