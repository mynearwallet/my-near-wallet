...
async function hasStorageDeposit(account: Wallet.Account, tokenId: string): Promise<boolean> {
  if (account && tokenId) {
    try {
      const storageState = await account.viewFunction(tokenId, "storage_balance_of", {
