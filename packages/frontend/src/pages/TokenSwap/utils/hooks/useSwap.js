...
const getAmountStats = ({ tokenIn, amountIn, tokenOut, amountOut, nearUsdPrice }) => {
  const tokenInId = fungibleTokenExchange.replaceNearIdIfNecessary(tokenIn.contractName);
  const tokenOutId = fungibleTokenExchange.replaceNearIdIfNecessary(tokenOut.contractName);
...
export default function useSwap({
      ...
      try {
        ...
        dispatch(
          updateTokensBalance({
            ...
            tokenIds: [
              fungibleTokenExchange.replaceNearIdIfNecessary(tokenIn.contractName),
              fungibleTokenExchange.replaceNearIdIfNecessary(tokenOut.contractName),
            ],
          }),
        );
  ...
  }, [dispatch, account, amountIn, poolId, tokenIn, tokenOut, minAmountOut, isNearTransformation]);
