...
export default class NonFungibleTokens {
  ...
  static getNumberOfTokens = ({ contractName, accountId }) => {
    return this.viewFunctionAccount.viewFunction(contractName, "nft_supply_for_owner", {
      account_id: accountId,
    });
  };

  static getToken = async (contractName, tokenId, base_uri) => {
    const token = await this.viewFunctionAccount.viewFunction(contractName, "nft_token", {
      token_id: tokenId,
    });
  ...
  static getTokens = async ({ contractName, accountId, base_uri, fromIndex = 0 }) => {
    ...
    try {
      ...
      tokens = await Promise.all(
        tokenIds.slice(fromIndex, TOKENS_PER_PAGE + fromIndex).map(async (token_id) => ({
