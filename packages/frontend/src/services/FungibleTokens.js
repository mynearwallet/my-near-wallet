...
export default class FungibleTokens {
  ...
  static async checkRegistration({ contractName, accountId }) {
    try {
      return await this.viewFunctionAccount.viewFunction(contractName, "check_registration", {
  ...
  static getParsedTokenAmount(amount, symbol, decimals) {
    const parsedTokenAmount =
      symbol === CONFIG.NEAR_ID ? parseNearAmount(amount) : parseTokenAmount(amount, decimals);
  ...
  static async getStorageBalance({ contractName, accountId }) {
    return await this.viewFunctionAccount.viewFunction(contractName, "storage_balance_of", {
      account_id: accountId,
    });
  }
  ...
  async getEstimatedTotalFees({ accountId, contractName } = {}) {
    if (contractName && accountId) {
      ...
      if (isRegistrationRequired) {
        ...
        return new BN(gasFeesWithStorage).add(new BN(CONFIG.FT_REGISTRATION_DEPOSIT)).toString();
      }
      ...
      if (isStorageDepositRequired) {
        ...
        return new BN(gasFeesWithStorage).add(new BN(CONFIG.FT_MINIMUM_STORAGE_BALANCE)).toString();
      }
    }
  ...
  async getWrapNearTx({ accountId, amount }) {
    ...
    actions.push(
      functionCall("near_deposit", {}, CONFIG.FT_STORAGE_DEPOSIT_GAS, parseNearAmount(amount)),
    );
