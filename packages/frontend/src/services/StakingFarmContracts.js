...
export default class StakingFarmContracts {
  ...
  static getFarms({ contractName, from_index, limit }) {
    return this.viewFunctionAccount.viewFunction(contractName, "get_farms", { from_index, limit });
  }
  ...
  static getUnclaimedRewards({ contractName, account_id, farm_id }) {
    return this.viewFunctionAccount.viewFunction(contractName, "get_unclaimed_reward", {
      account_id,
      farm_id,
    });
  }
  ...
  static isFarmingValidator(accountId) {
    return (
      getValidationVersion(CONFIG.NODE_URL.indexOf(MAINNET) > -1 ? MAINNET : TESTNET, accountId) ===
      FARMING_VALIDATOR_VERSION
    );
  }

  static hasUnclaimedRewards = ({ contractName, account_id, from_index = 0, limit = 300 }) => {
    return (
