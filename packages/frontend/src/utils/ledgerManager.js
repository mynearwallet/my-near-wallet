...
async function createClient(transport) {
    return {
        ...
        async signMessage(transactionData, path, isNep413) {
            ...
            const INSTRUCTION_SIGNMESSAGE = 2;
            const INSTRUCTION_SIGNMESSAGE_NEP413 = 7;
            ...
            for (let offset = 0; offset < allData.length; offset += CHUNK_SIZE) {
                ...
                const response = await this.transport.send(
                    0x80,
                    isNep413 ? INSTRUCTION_SIGNMESSAGE_NEP413 : INSTRUCTION_SIGNMESSAGE,
                    isLastChunk ? 0x80 : 0,
                    networkId,
                    chunk
                );
