...
const { setLedgerTxSigned, showLedgerModal, handleShowConnectModal, checkAndHideLedgerModal } =
  ledgerActions;
...
export default class Wallet {
  ...
  getAccountsLocalStorage() {
    return (this.accounts = JSON.parse(localStorage.getItem(KEY_WALLET_ACCOUNTS) || "{}"));
  }
  ...
  async loadAccount(limitedAccountData = false) {
    if (!this.isEmpty()) {
      const accessKeys = limitedAccountData ? [] : (await this.getAccessKeys()) || [];
      const ledgerKey = accessKeys.find((key) => key.meta.type === "ledger");
  ...
  isMultisigKeyInfoView(accountId, keyInfoView) {
    ...
    return (
      ...
      isEqual(method_names, ["add_request", "add_request_and_confirm", "delete_request", "confirm"])
    );
  }

  async addExistingAccountKeyToWalletKeyStore(accountId, keyPair, ledgerHdPath) {
    const keyType = await this.getPublicKeyType(accountId, keyPair.getPublicKey().toString());
  ...
  async createNewAccountWithNearContract({
    ...
    if (!createResult) {
      throw new WalletError("Creating account has failed", "createAccount.returnedFalse", {
        transactionHash,
      });
    }
  }
      ...
      await Promise.all(
        accountIds.map(async (accountId) => {
          try {
            ...
            return accountKeys.find(({ public_key }) => public_key === publicKey.toString())
              ? accountId
              : null;
          } catch (error) {
  ...
  async recoverAccountSecretKey(secretKey, accountId, shouldCreateFullAccessKey) {
    ...
    await Promise.all(
      accountIds.map(async (accountId, i) => {
        ...
        account.inMemorySigner = account.connection.signer = new nearApiJs.InMemorySigner(
          tempKeyStore,
        );
    ...
    if (accountIdsSuccess.length) {
      ...
      if (lastAccount) {
        this.accountId = localStorage.getItem(KEY_ACTIVE_ACCOUNT_ID) || "";
        store.dispatch(
          redirectTo(`/profile/${lastAccount.accountId}`, { globalAlertPreventClear: true }),
        );
  ...
  async signAndSendCalimeroTransaction(
    ...
    for (const { receiverId, nonce, blockHash, actions } of transactions) {
      ...
      ({ status, transaction } = await calimeroConnection.provider.sendTransaction(
        signedTransaction,
      ));
