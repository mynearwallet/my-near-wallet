...
async function getAccountBalance(limitedAccountData = false) {
  ...
  await Promise.all(
    stakingDeposits.map(async ({ validator_id }) => {
      const validatorBalance = new BN(
        await this.wrappedAccount.viewFunction(validator_id, "get_account_total_balance", {
  ...
  try {
    ...
    if (startTimestampBN.lte(dateNowBN)) {
      ...
      const endTimestamp = (hasBrokenTimestamp ? new BN(transfersTimestamp) : startTimestampBN).add(
        releaseDurationBN,
      );
    ...
    const isAccDeletable =
      lockedAmount.isZero() && stakedBalanceLockup.isZero() && !hasUnclaimedTokenBalance;
