...
test("returns base string and both conditionals", () => {
  expect(classNames(["first", { "conditional-one": true, "conditional-two": true }])).toBe(
    "first conditional-one conditional-two",
  );
});

test("returns base string and second conditional", () => {
  expect(classNames(["first", { "conditional-one": false, "conditional-two": true }])).toBe(
