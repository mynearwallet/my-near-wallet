...
export const formatNearAmount = (amount: number | string) => {
...
export const showInYocto = (amount: string) => {
  return `${formatWithCommas(amount)} yoctoNEAR`;
};

export function formatErrorBalance(message: string) {
  const regExp = /\d* yoctoNEAR/;
  const yoctoSubString = message.match(regExp);
  if (yoctoSubString) {
...
export const getTotalBalanceInFiat = (mainTokens, currentLanguage: string) => {
  const totalAmount = mainTokens
    .map((el) => {
      const USD = el.fiatValueMetadata.usd;
      const balance = el.balance;
      return el.contractName
        ? getRoundedBalanceInFiat(balance, USD, true, el.onChainFTMetadata.decimals)
...
export const getTotalBalanceFromFungibleTokensListUSD = (fungibleTokensList: Wallet.Token[]) => {
