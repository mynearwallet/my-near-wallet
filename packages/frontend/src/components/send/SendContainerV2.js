...
const SendContainerV2 = ({
  ...
  useEffect(() => {
    ...
    let targetToken =
      fungibleTokens.find(
        ({ contractName }) => contractName && contractName === selectedToken.contractName,
      ) ||
      fungibleTokens.find(
        ({ onChainFTMetadata }) =>
      ...
      case VIEWS.SUCCESS:
        return (
          <Success
            ...
            onClickGoToExplorer={() =>
              window.open(`${explorerUrl}/transactions/${transactionHash}`, "_blank")
            }
          />
        );
      default:
  ...
  return (
    <StyledContainer
      className={classNames(["small-centered", { "showing-banner": showNetworkBanner }])}
    >
