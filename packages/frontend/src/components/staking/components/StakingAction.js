...
export default function StakingAction({
  ...
  const stakeActionAllowed =
    hasStakeActionAmount && !invalidStakeActionAmount && !success && !validatorMightHaveFAK;
      ...
      <>
        ...
        <div className='desc' data-test-id="stakingSuccessMessage">
          <SafeTranslate
            ...
            data={{
              amount: getNearAndFiatValue(parseNearAmount(displayAmount), nearTokenFiatValueUSD),
            }}
          />
        </div>
        {validator && (
