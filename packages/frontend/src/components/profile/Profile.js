...
const Profile = ({ match }) => {
  ...
  const twoFactor =
    has2fa && userRecoveryMethods && userRecoveryMethods.filter((m) => m.kind.includes("2fa"))[0];
      ...
      <div className='split'>
        <div className='left'>
          ...
          {profileBalance ? (
            ...
            <SkeletonLoading height='323px' show={!profileBalance} number={2} />
          )}
          {profileBalance?.lockupIdExists && (
            <SkeletonLoading height='323px' show={hasLockup === undefined} number={1} />
          )}
        ...
        {isOwner && (
          <div className='right'>
            ...
            {!CONFIG.IS_MAINNET && !account.ledgerKey && !isMobile() && <MobileSharingWrapper />}
          </div>
        )}
