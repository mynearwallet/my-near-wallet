...
const WalletMigration = ({ open, onClose }) => {
  ...
  const onContinue = useCallback(async () => {
    let url = "";
    url = await encodeAccountsToURL(availableAccounts, state.migrationKey, state.wallet);
  ...
  return (
    <div>
      {state.activeView === WALLET_MIGRATION_VIEWS.DISABLE_2FA && (
        <Disable2FAModal onClose={onClose} handleSetActiveView={handleSetActiveView} />
      )}
      ...
      {state.activeView === WALLET_MIGRATION_VIEWS.MIGRATE_ACCOUNTS && (
        <MigrateAccounts accounts={availableAccounts} onContinue={onContinue} onClose={onClose} />
      )}
    </div>
  );
};
