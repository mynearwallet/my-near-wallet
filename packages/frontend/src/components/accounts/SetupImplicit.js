...
class SetupImplicit extends Component {
  ...
  handleContinue = async () => {
    ...
    await Mixpanel.withTracking(
      ...
      async () => {
        await dispatch(
          createAccountFromImplicit({ accountId: newAccountId, implicitAccountId, recoveryMethod }),
        ).unwrap();
      },
      () => {
  ...
  render() {
    ...
    if (accountFunded) {
      return (
        <StyledContainer className='small-centered funded'>
          <h1>
            <Translate id='account.createImplicit.post.title' />
          </h1>
          <h2>
            <Translate id='account.createImplicit.post.descOne' />
          </h2>
          <h2>
            <b>
              <Translate id='account.createImplicit.post.descTwo' />
            </b>
          </h2>
          {!creatingAccount && (
            <AccountFunded
    ...
    return (
      <StyledContainer className='small-centered funded'>
        <h1>
          <Translate id='account.createImplicit.pre.title' />
        </h1>
        <h2>
          <SafeTranslate
            id='account.createImplicit.pre.descOne'
            data={{
              amount: getNearAndFiatValue(CONFIG.MIN_BALANCE_TO_CREATE, nearTokenFiatValueUSD),
            }}
          />
        </h2>
        <FormButton
        ...
        />
        <Divider />
        <FundWithUtorg accountId={implicitAccountId} amount={CONFIG.MIN_BALANCE_TO_CREATE} />
        {moonpayAvailable && (
          <>
            <Divider />
            <FundWithMoonpay moonpaySignedURL={moonpaySignedURL} />
          </>
        )}
        {whereToBuy && (
          <WhereToBuyNearModal
...
const mapStateToProps = (
  state,
  { match: { params: { accountId, implicitAccountId, recoveryMethod } } },
) => ({
...
const SetupImplicitWithRouter = connect(mapStateToProps)(withRouter(SetupImplicit));
