...
const VerifyAccountWrapper = () => {
    ...
    const [showFundWithCreditCardOption, setShowFundWithCreditCardOption] =
        useState(false);
    ...
    const getIdentityKey = () =>
        activeVerificationOption === 'email' ? verificationEmail : verificationNumber;
            ...
            <EnterVerificationCode
                ...
                onConfirm={async (verificationCode) => {
                    const recaptchaToken = await handleReCaptchaVerify(
                        'verifiedIdentityCreateFundedAccount'
                    );
                    if (!recaptchaToken) {
                        dispatch(
                            showCustomAlert({
                    ...
                    try {
                        ...
                        if (e.code === 'identityVerificationCodeInvalid') {
                            ...
                            e.code === 'identityVerificationEmailProviderInvalid'
                        ) {
                            dispatch(
                                showCustomAlert({
                                    success: false,
                ...
                onResend={async () => {
                    const recaptchaToken = await handleReCaptchaVerify(
                        'verifiedIdentityCreateVerificationMethod'
                    );
                    if (!recaptchaToken) {
                        dispatch(
                            showCustomAlert({
    ...
    return (
        <VerifyAccount
            ...
            handleContinue={async () => {
                if (
                    activeVerificationOption === 'email' ||
                    activeVerificationOption === 'phone'
                ) {
                    try {
                        const recaptchaToken = await handleReCaptchaVerify(
                            'verifiedIdentityCreateVerificationMethod'
                ...
                if (activeVerificationOption === 'existingAccount') {
                    return dispatch(
                        redirectTo(`/fund-with-existing-account${location.search}`)
                    );
                }
