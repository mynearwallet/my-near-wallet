...
const AccountImportModal = ({ account, onSuccess, onFail }) => {
  ...
  const addFAKTransaction = useMemo(() => {
    try {
      return {
        ...
        actions: [
          transactions.addKey(KeyPair.fromString(account.key), transactions.fullAccessKey()),
        ],
      };
    } catch (error) {
      return null;
  ...
  useEffect(() => {
    ...
    wallet.getBalance(account.accountId).then(({ available }) => setAccountBalance(available));
  }, [account]);
  ...
  const addKeyToWalletKeyStore = useCallback(() => {
    ...
    if (keyPair) {
      wallet
        .addExistingAccountKeyToWalletKeyStore(account.accountId, keyPair, account.ledgerHdPath)
  ...
  return (
    <Modal isOpen={account} modalSize='md' modalClass='slim' onClose={() => {}} disableClose>
      {showTxDetails ? (
        ...
        <ModalContainer>
          ...
          <SignTransaction
            ...
            estimatedFees={
              keyType === WalletClass.KEY_TYPES.FAK ? estimatedAddFAKTransactionFees : "0"
            }
