...
import { redirectToApp, redirectTo, clearAccountState } from "../../../redux/actions/account";
...
const SignInLedgerWrapper = (props) => {
  ...
  const accountsApproved = signInWithLedgerKeys.reduce(
    (a, accountId) => (signInWithLedgerState[accountId].status === "success" ? a + 1 : a),
    0,
  );
  const accountsError = signInWithLedgerKeys.reduce(
    (a, accountId) => (signInWithLedgerState[accountId].status === "error" ? a + 1 : a),
    0,
  );
  const accountsRejected = signInWithLedgerKeys.reduce(
    (a, accountId) => (signInWithLedgerState[accountId].status === "rejected" ? a + 1 : a),
    0,
  );
