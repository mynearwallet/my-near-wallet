...
describe('Staking flow', () => {
    ...
    test('navigates to staking page with correct balance', async ({ page }) => {
        ...
        await expect(page.locator('data-test-id=accountSelectStakedBalance')).toHaveText(
            '0 NEAR'
        );
    });
    test('correctly searches for validators', async ({ page }) => {
        ...
        await expect(
            page.locator('data-test-id=stakingPageValidatorFoundLabel')
        ).toBeVisible();
    ...
    test('correctly selects and stakes with validator', async ({ page }) => {
        ...
        let currentlyDisplayedWalletBalance =
            await stakeUnstakePage.getCurrentlyDisplayedBalance();
        await stakeUnstakePage.clickStakeButton();
        ...
        await expect(page.locator('data-test-id=validatorNamePageTitle')).toHaveText(
            new RegExp(`${validatorName}`)
        );
        ...
        currentlyDisplayedWalletBalance =
            await stakeUnstakePage.getCurrentlyDisplayedBalance();
        ...
        ).toHaveText(new RegExp(testStakeAmount.toString()));
