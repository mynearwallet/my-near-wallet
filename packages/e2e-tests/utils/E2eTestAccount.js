...
class E2eTestAccount {
    ...
    async initialize() {
        await nearApiJsConnection.setKeyPairFromSeedPhrase({
     ...
     * @returns
     */
    async createTestLockupSubAccountInstance({
        amount,
        release_duration,
        lockup_timestamp,
        vesting_schedule,
        ...
        const vesting_schedule_config =
            vesting_schedule || FULLY_VESTED_CONFIG.vesting_schedule;

        return new E2eTestAccount(
            lockupSubaccountId,
            lockupSubaccountSeedphrase,
            this.nearApiJsAccount
        ).create({
            amount: amount || '5.0',
            ...
            initArgs: {
                ...
                lockup_duration: '0',
                lockup_timestamp:
                    lockup_timestamp || FULLY_VESTED_CONFIG.lockup_timestamp,
                transfers_information: {
                    TransfersEnabled: {
                        transfers_timestamp: new BN(Date.now())
                            .mul(new BN('1000000'))
                            .sub(minuteInNanosBN)
                            .toString(), // 1 minute ago
                    },
                },
                release_duration:
                    release_duration || FULLY_VESTED_CONFIG.release_duration,
    ...
    connectOrCreate(...creationArgs) {
        return this.initialize().catch((e) => {
            if (
                new RegExp(`${this.accountId} does not exist while viewing`).test(
                    e.message
                )
            ) {
    ...
    async delete() {
        if (this.isCreated) {
            ...
            await this.nearApiJsAccount.deleteAccount(
                this.parentNearApiJsAccount.accountId
            );
        }
    }
    ...
    async getTokenBalance(contractId) {
        await this.connectToNearApiJs();
        return this.nearApiJsAccount.viewFunction(contractId, 'ft_balance_of', {
            account_id: this.accountId,
        });
    }
    async getAmountStakedWithValidator(validatorAccountId) {
        const balanceString = await this.nearApiJsAccount.viewFunction(
            validatorAccountId,
