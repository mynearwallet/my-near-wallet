...
class WalletE2eLogsReporter {
  ...
  getTestsForWorkerIndex(idx) {
    return this.suite
      .allTests()
      .filter(({ results }) => results.some(({ workerIndex }) => workerIndex === idx));
  }
  async printWorkerExpenses() {
    this.workerExpenseLogs
      ...
      .forEach(({ workerBankAccount, amountSpent, workerIndex }) => {
        this.logger.info(
          `amount spent by worker acc ${workerBankAccount}: ${formatNearAmount(amountSpent)} Ⓝ`,
        );
        this.logger.info(
    ...
    this.logger.info(
      `Total amount spent: ${formatNearAmount(
        this.workerExpenseLogs
          .reduce((acc, { amountSpent }) => new BN(amountSpent).add(acc), new BN(0))
          .toString(),
      )} Ⓝ`,
    );
    const bankAccount = await getBankAccount();
