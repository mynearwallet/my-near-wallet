...
describe("NFT flow", () => {
  ...
  test("able to view and send NFT", async ({ page }) => {
    ...
    await expect(
      page.getByText("Example NEAR non-fungible token1")
    ).toBeVisible();
    ...
    const tokenForReceiver = await nftAccountManager.nftContractAccount.nearApiJsAccount?.viewFunction(
      nftAccountManager.nftContractAccount.accountId,
      "nft_tokens_for_owner",
      { account_id: nftAccountManager.nftReceiverAccount.accountId }
    );

    expect(tokenForReceiver[0].owner_id).toBe(nftAccountManager.nftReceiverAccount.accountId)
  });
});
